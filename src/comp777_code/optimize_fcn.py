...
def optimize_image_with_torch(x, fcn, fcn_pars=None, nr_of_iterations=100, optimizer=None, record_history=False, display_loss=True):
    ...
    for i in range(nr_of_iterations):
        ...
        y = fcn(x, fcn_pars)
        if display_loss:
            print('Iter {}: y={}'.format(i,y.item()))

        if record_history:
            history.append( (float(i),y.item()))
